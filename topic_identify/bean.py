...
class Article:
    ...
    def pre_process(self, postagger, stop_words, n_keywords, multi_title):
        ...
        self.title_content_effective_words = [w for w, t in title_content_word_tag_dict.items() if t in self.post_list and w not in stop_words]
        self.title_content_effective_word_tfidfs = [(word, tfidf) for word, tfidf in title_content_word_tfidfs if word in self.title_content_effective_words]
        self.title_content_effective_word_posts = [(word, postag) for word, postag in title_content_word_tag_dict.items() if
        ...
        self.title_effective_words = [w for w, t in title_word_tag_dict.items() if t in self.post_list and w not in stop_words]
        self.title_effective_word_tfidfs = [(word, tfidf) for word, tfidf in title_word_tfidfs if word in self.title_effective_words]
        ...
        self.content_effective_words = [w for w, t in content_word_tag_dict.items() if t in self.post_list and w not in stop_words]
        self.content_effective_word_tfidfs = [(word, tfidf) for word, tfidf in content_word_tfidfs if word in self.content_effective_words]
